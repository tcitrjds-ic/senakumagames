...
| `make_pixel_chars.py` | せなくま美術館の歩行キャラ（せなくま・キノピオ・マリオ・ルイージ・ピーチ）のドット絵シートを生成（正面/背面/横 の3行 × 立ち/歩きA/歩きB の3列。セルは既定24×32マス＝72×96px、キャラごとに `"cell"` で上書き可でせなくまは32×44マス＝96×132px） |
| `make_gallery_assets.py` | せなくま美術館の素材を生成（ピーチ城風の外観画像と、3Dホール用のチェック床・太陽モザイク・石壁・空と丘の壁画・天井・扉・窓・絨毯テクスチャ） |
