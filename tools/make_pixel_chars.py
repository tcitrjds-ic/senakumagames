...
各キャラを 24×32 マスのセルで「正面・背面・横（右向き）」の3方向 × 「立ち・歩きA・歩きB」の
3コマ = 9コマのシート（72×96 px、1マス=1px。ゲーム側で NEAREST 拡大）として出力する。
横向きの左向きはゲーム側で左右反転。歩きコマは立ち絵から足の動きを機械生成する。
...
使い方:
    pip install Pillow
    python tools/make_pixel_chars.py
"""
